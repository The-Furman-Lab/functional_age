...
# features (forced in, penalty 0) plus the proteins selected by the domain's
# proteomic surrogate (penalised). Out-of-fold linear predictors give the
# discrimination gain from adding proteomics over the clinical model.
#
# Run from the repository root, with the domain id 1..5:
#   Rscript code/03_clinical_proteomics_models.R <id>
...
selected <- do.call(rbind, lapply(list.files(out_path("models", "proteomics"), full.names = TRUE),
...
proteins <- selected$gene[selected$domain == str_to_sentence(d)]
features <- cbind(survival %>% dplyr::select(time, status, age, sex), domain, proteomics[rownames(survival), proteins])
...
write_rds(tibble(domain = d, coef = list(coef), pred = list(pred)),
          out_path("models", "clinical_proteomics", paste0(d, ".rds")))
